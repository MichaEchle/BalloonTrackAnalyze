...
List<(int pilotNumber, double result)> results = [];
...
List<(int pilotNumber, double result, int score)> scores = ScoreCalculation.CalculateScores(results, winningType);

foreach ((int pilotNumber, double result, int score) score in scores)
{
...
for (int index = 1; index < lines.Length; index++)
{
    ...
    int score = scores.FirstOrDefault(x => x.pilotNumber.ToString() == columns[pilotNumberColumn]).score;
