...
namespace BLC2021
{
    public partial class BLC2021TaskSheet2 : Form
    {
            ...
            for (int northingFactor = -1; northingFactor <= 1; northingFactor++)
            {
                for (int eastingFactor = -1; eastingFactor <= 1; eastingFactor++)
                {
                    if (northingFactor == 0 && eastingFactor == 0)
                        continue;
                    CoordinateSharp.UniversalTransverseMercator utmTempGoal = new($"{latZone}{longZone}", easting + (distanceFromDeclaredGoal * eastingFactor), northing + (distanceFromDeclaredGoal * northingFactor));
        ...
        private async Task<bool> ParseFilesAndCalculateResultsAsync(string[] igcFiles, HesitationWaltzTask task4_1, HesitationWaltzTask task4_2, HesitationWaltzTask task4_3, HesitationWaltzTask task5)
        {
            ...
            {
                List<(string pilotIdenifier, int pilotNumber, double result_T4, double result_T5)> results = [];
                try
                {
                    ...
                    foreach (string igcFile in igcFiles)
                    {
                        ...
                        {
                            if (!task4_1.CalculateResults(track, true, out double result_Task4_1))
                            {
                            ...
                            if (!task4_2.CalculateResults(track, true, out double result_Task4_2))
                            {
                                Logger?.LogError("Failed to parse files and calculate results: Failed to calculate result at Task 4 for Pilot No '{pilotNumber}'", track.Pilot.PilotNumber);
                            ...
                            if (!task4_3.CalculateResults(track, true, out double result_Task4_3))
                            {
                                Logger?.LogError("Failed to parse files and calculate results: Failed to calculate result at Task 4 for Pilot No '{pilotNumber}'", track.Pilot.PilotNumber);
                    ...
                    using (ExcelPackage package = new(resultsFileInternal))
                    {
                        ...
                        foreach ((string pilotIdentifier, int pilotNumber, double result_T4, double result_T5) in results)
                        {
                            wsResults.Cells[pilotNumber + 1, 1].Value = pilotIdentifier;
                            wsResults.Cells[pilotNumber + 1, 2].Value = pilotNumber;
                            wsResults.Cells[pilotNumber + 1, 3].Value = Math.Round(result_T4, 0, MidpointRounding.AwayFromZero);
                            wsResults.Cells[pilotNumber + 1, 4].Value = Math.Round(result_T5, 0, MidpointRounding.AwayFromZero);
                    ...
                    using (ExcelPackage package = new(resultsFileProvisional))
                    {
                        ...
                        foreach ((string pilotIdentifier, int pilotNumber, double result_T4, double result_T5) in results)
                        {
                            ...
                            wsResults.Cells[pilotNumber + 1, 5].Value = Math.Round(result_T4, 0, MidpointRounding.AwayFromZero);
                            wsResults.Cells[pilotNumber + 1, 6].Value = Math.Round(result_T5, 0, MidpointRounding.AwayFromZero);
